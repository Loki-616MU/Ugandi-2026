...
function initAnimations() {
    ...
    window._prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    ...
    if (window._prefersReducedMotion) {
        // Skip heavy canvas animations entirely
        initScrollProgress();
        initNavHighlight();
        initCosmicSection();
        initDangerSection();
...
function initStarField() {
    ...
    function resize() {
        ...
        stars = [];
        const starCount = window._isMobile ? 60 : 200;
...
function initYugasSection() {
    ...
    if (krishnaVideo && overlay && fullscreenVideo) {
        ...
        ScrollTrigger.create({
            ...
            start: 'top 70%',
            onEnter: () => {
                // Skip fullscreen takeover on mobile — too disruptive
                if (window._isMobile) {
...
function initUnderwaterBubbles() {
    ...
    ScrollTrigger.create({
        ...
        onToggle: (self) => {
            if (self.isActive) {
                const bubbleInterval = window._isMobile ? 1500 : 600;
...
function initRainCanvas() {
    ...
    function createRainDrops() {
        drops = [];
        const dropCount = window._isMobile ? 40 : 120;
